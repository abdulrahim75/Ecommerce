...
const { GoogleGenerativeAIEmbeddings } = require("@langchain/google-genai"); // For creating vector embeddings from text using Gemini
const { ChatGoogleGenerativeAI } = require("@langchain/google-genai"); // Google's Gemini AI model
    ...
    const itemLookupTool = tool(
      ...
      // Tool metadata and schema definition
      {
        ...
        schema: z.object({
          ...
          n: z.number().optional().default(10).describe("Number of results to return"), // Optional number parameter with default
        }),
      }
    );
